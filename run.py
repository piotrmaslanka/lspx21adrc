# coding=UTF-8
"""
This program can be used to change address of a LSPX-21 module over a serial port

Copyright (c) DMS Serwis 2015
"""

from __future__ import print_function
import serial, struct


READ_ADDRESS_QUERY = '\x00\x71\x00\x00\x00\x01\xBD\xD0'
CRCLO = []
CRCHI = []
WRITE_ADDRESS_QUERY = '\x0x\x74\x00\x00\x00\x03\x06\x00%s\x00%s\x00%s'


def cCRC(st):   # str -> str
    crclo, crchi = 0xFF, 0xFF
    for ch in st:
        index = crchi ^ ord(ch)
        crchi = crclo ^ CRCHI[index]
        crclo = CRCLO[index]

    return struct.pack('<H', (crclo<<8)+crchi)


print('Please make sure LSPX-21 is set to 9600 baud + no parity')

port = serial(raw_input('Enter serial port name (eg. COM1)'), baudrate=9600)

print('Attempting to read current address...')

port.write(READ_ADDRESS_QUERY)
current_address = ord(port.read(size=8)[3])

print('Current address is %s' % (current_address, ))

filter, baudrate, address = 0, 0, int(raw_input('Enter new MODBUS address'))

writeCmd = WRITE_ADDRESS_QUERY % (filter, baudrate, address)
port.write(writeCmd + cCRC(writeCmd))

if port.read(8).startswith('\x02\x74\x00\x00\x00\x03'):
    print('Reprogramming successful.')
